...
def train(config: DictConfig) -> Optional[float]:
    ...
    datamodule: LightningDataModule = hydra.utils.instantiate(config.datamodule, model_name_or_path=config.model_model_nlp.model_name_or_path)
    datamodule.setup("fit")


    # Init lightning model
    log.info(f"Instantiating model <{config.model._target_}>")
    model = hydra.utils.instantiate(config.model_model_nlp,
    ...
    # trainer.checkpoint_callback.best_model_path = "/users/janner/goldiprox-hydra/logs/runs/2022-04-10/14-33-58/checkpoints/epoch_002.ckpt"
    ...
    def evaluate_and_save_model_from_checkpoint_path(checkpoint_path, name):
        ...
        irreducible_loss_and_checks = utils.compute_losses_with_sanity_checks_nlp(
