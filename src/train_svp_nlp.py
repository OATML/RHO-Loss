...
from src.models.SVPModels_nlp import ForgettingEventsModel, OneModel_SVP
...
def train(config: DictConfig) -> Optional[float]:
    ...
    datamodule: LightningDataModule = hydra.utils.instantiate(config.datamodule, model_name_or_path=config.model_proxy_model_nlp.model_name_or_path)
    ...
    if config.model.pretrained_proxy:
        ...
        log.info(f"Instantiating SVP Proxy Model <{config.model_proxy_model_nlp._target_}>")
        proxy_model_nn = hydra.utils.instantiate(config.model_proxy_model_nlp,
    ...
    datamodule: LightningDataModule = hydra.utils.instantiate(config.datamodule, model_name_or_path=config.model_proxy_model_nlp.model_name_or_path, sequence=sequence, _convert_="partial")
    datamodule.setup("fit")
    ...
    # Init lightning trainer
    log.info(f"Instantiating trainer for main model <{config.trainer._target_}>")
    trainer: Trainer = hydra.utils.instantiate(
    ...
    large_model_nn = hydra.utils.instantiate(config.model_large_model_nlp,
        num_labels=datamodule.num_labels,
        eval_splits=datamodule.eval_splits,
        task_name=datamodule.task_name,
    )
